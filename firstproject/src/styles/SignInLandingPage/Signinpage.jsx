...
export function SignInPage({ onNavigateToRegister }) {
  ...
  const handleForgotPassword = () => {
    const normalised = forgotEmail.trim().toLowerCase();
    const DEMO = [
      { email: "ub07100@st.habib.edu.pk",     password: "Student@123",    name: "Ushna Batool"    },
      { email: "sarah.ahmed@st.habib.edu.pk", password: "FocusPeer@123", name: "Sarah Ahmed"     },
      { email: "fatima.khan@habib.edu.pk",    password: "OAP@123",        name: "Dr. Fatima Khan" },
      { email: "sara.ali@habib.edu.pk",       password: "Ehsas@123",      name: "Sara Ali"        },
      { email: "dr.zainab@habib.edu.pk",      password: "Wellness@123",   name: "Dr. Zainab"      },
      { email: "dr.ahmed@habib.edu.pk",       password: "Faculty@123",    name: "Dr. Ahmed"       },
    ];
    const demo = DEMO.find(d => d.email.toLowerCase() === normalised);
    if (demo) { setForgotResult({ found: true, password: demo.password, name: demo.name }); return; }
                    ...
                    <>
                      ...
                      <form onSubmit={handleSignIn} style={{ display: "flex", flexDirection: "column", gap: 15 }}>
                        ...
                        <div style={{ textAlign: "right", marginTop: 4 }}>
                          <button type="button" onClick={() => { setShowForgot(true); setForgotEmail(email); setForgotResult(null); }}
                            style={{ background: "none", border: "none", cursor: "pointer", color: C.purple500, fontSize: 12.5, fontWeight: 600, padding: 0 }}>
                            Forgot password?
                          </button>
                        </div>
                        ...
                        <button type="submit" disabled={signInLoading} style={btnStyle(signInLoading)}
                    ...
                    suSuccess ? (
                      ...
                      <>
                        ...
                        <form onSubmit={handleSignUp} style={{ display: "flex", flexDirection: "column", gap: 13 }}>
                          ...
                          <button type="submit" disabled={suLoading} style={btnStyle(suLoading)}
      ...
      {showForgot && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
          <div style={{ background: "#fff", borderRadius: 24, boxShadow: "0 24px 60px rgba(0,0,0,0.15)", width: "100%", maxWidth: 420, padding: "32px 36px" }}>
            <h3 style={{ fontSize: 20, fontWeight: 800, color: C.purple800, margin: "0 0 6px" }}>Forgot Password?</h3>
            ...
            {forgotResult && (
              forgotResult.found ? (
                <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(34,197,94,0.08)", border: "1.5px solid rgba(34,197,94,0.25)", marginBottom: 16 }}>
                  <p style={{ margin: "0 0 4px", fontSize: 13, color: "#5a4a61", fontWeight: 600 }}>Hi, {forgotResult.name}!</p>
                  <p style={{ margin: "0 0 6px", fontSize: 12.5, color: "#9575a3" }}>Your password is:</p>
                  <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#22c55e", letterSpacing: "0.04em" }}>{forgotResult.password}</p>
                </div>
              ) : (
                <div style={{ padding: "12px 16px", borderRadius: 12, background: "#FFF1F2", border: "1.5px solid #FECACA", marginBottom: 16 }}>
                  <p style={{ margin: 0, fontSize: 13, color: "#B91C1C", fontWeight: 600 }}>⚠️ No account found for this email.</p>
                </div>
              )
            )}

            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={() => { setShowForgot(false); setForgotResult(null); }}
                style={{ flex: 1, padding: "11px", borderRadius: 12, border: `1.5px solid ${C.purple200}`, background: "#fff", color: C.purple600, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                Close
              </button>
              <button onClick={handleForgotPassword}
                style={{ flex: 1, padding: "11px", borderRadius: 12, border: "none", background: `linear-gradient(90deg, ${C.btnFrom} 0%, ${C.btnTo} 100%)`, color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
                Find Password
              </button>
            </div>
          </div>
