...
// import React, { useState, useEffect } from "react";
// import { useNavigate } from "react-router-dom";
// import { Star, Award, TrendingUp, Calendar, ChevronLeft, ChevronRight, Sparkles, Brain, Grid3x3, Clock, Edit2, Check, X } from "lucide-react";
...
//   // ── Single source of truth: Database Fetching ───────────────────────────
//   const [upcomingAssignments, setUpcomingAssignments] = useState([]);

//   const loadFromDatabase = async () => {
//     try {
//       // Using Ushna's ID directly for now to ensure it loads
...
//   useEffect(() => {
//     loadFromDatabase();
//     window.addEventListener("focus", loadFromDatabase);
//     window.addEventListener("eisenhowerSaved", loadFromDatabase);
//     // Poll the database every 2 seconds to keep it fresh
//     const interval = setInterval(loadFromDatabase, 2000); 
    
//     return () => {
//       window.removeEventListener("focus", loadFromDatabase);
//       window.removeEventListener("eisenhowerSaved", loadFromDatabase);
//       clearInterval(interval);
//     };
//   }, []);

//   // Save todos separately
//   useEffect(() => {
//     localStorage.setItem("dashboardTodos", JSON.stringify(tasks));
//   }, [tasks]);
...
//   const assignmentsOnDate = (dateStr) => upcomingAssignments.filter(a => a.dueDate === dateStr);

//   const sortedUpcoming = [...upcomingAssignments]
//     .filter(a => a.status !== "completed" && a.status !== "Completed" && (a.progress ?? 0) < 100)
//     .sort((a, b) => {
//       const order = { "High Priority": 0, "Medium Priority": 1, "Low Priority": 2 };
//       if (order[a.priority] !== order[b.priority]) return (order[a.priority] ?? 2) - (order[b.priority] ?? 2);
//       return new Date(a.dueDate || "9999") - new Date(b.dueDate || "9999");
...
//   const handleDeleteTask = (id) => setTasks(tasks.filter(t => t.id !== id));
//   const handleEnterKey = (e) => e.key === "Enter" && handleAddTask();
//   const startEditing = (task) => { setEditingTaskId(task.id); setEditingText(task.text); };
//   const saveEdit = (id) => {
//     if (editingText.trim()) setTasks(tasks.map(t => t.id === id ? { ...t, text: editingText.trim() } : t));
...
//                   <Star className="h-5 w-5" style={{ color: "#f8bbd0" }} />
//                   <h3 className="text-xl font-semibold" style={{ color: "#5a4a61" }}>My To-Do's</h3>
//                 </div>
...
//                           className="flex-1 text-sm px-2 py-1 rounded-lg focus:outline-none"
//                           style={{ background: "#ffffff", border: "1px solid #b39ddb", color: "#5a4a61" }}
//                         />
//                       ) : (
//                         <span className="flex-1 text-sm"
//                           style={{ textDecoration: task.completed ? "line-through" : "none", color: task.completed ? "#9575a3" : "#5a4a61" }}>
...
//                         {editingTaskId === task.id ? (
//                           <>
//                             <button onClick={() => saveEdit(task.id)} className="w-7 h-7 rounded-full flex items-center justify-center hover:opacity-80" style={{ background: "#e8f5e9" }}>
//                               <Check className="h-3.5 w-3.5" style={{ color: "#2e7d32" }} />
//                             </button>
//                             <button onClick={cancelEdit} className="w-7 h-7 rounded-full flex items-center justify-center hover:opacity-80" style={{ background: "#fce4ec" }}>
//                               <X className="h-3.5 w-3.5" style={{ color: "#c62828" }} />
//                             </button>
//                           </>
//                         ) : (
//                           <>
//                             <button onClick={() => startEditing(task)} className="w-7 h-7 rounded-full flex items-center justify-center hover:opacity-80" style={{ background: "#e3f2fd" }}>
//                               <Edit2 className="h-3.5 w-3.5" style={{ color: "#1565c0" }} />
//                             </button>
//                             <button onClick={() => handleDeleteTask(task.id)} className="w-7 h-7 rounded-full flex items-center justify-center hover:opacity-80" style={{ background: "#fce4ec" }}>
...
//                     style={{ background: "#fdf7fd", border: "1px solid rgba(179,157,219,0.2)", color: "#5a4a61" }} />
//                   <button onClick={handleAddTask}
//                     className="w-11 h-11 rounded-xl text-white flex items-center justify-center hover:opacity-90 text-xl font-medium"
//                     style={{ background: "#b39ddb" }}>+</button>
//                 </div>
//               </div>
...
function Dashboard() {
  ...
  // ── Calendar Data (Dots & Rings) ─────────────────────────────────────────
  const [upcomingAssignments, setUpcomingAssignments] = useState([]);
  
  // ── Main Tasks (Upcoming List) ───────────────────────────────────────────
  const [mainTasks, setMainTasks] = useState([]);

  // ✅ FETCH BOTH ROUTES INDEPENDENTLY!
  const loadDashboardData = async () => {
    try {
      const userId = "a1111111-1111-1111-1111-111111111111"; // Ushna's ID
      
      // 1. Fetch Calendar Data (Contains subtasks so dots show up on the calendar)
      const calRes = await fetch(`http://127.0.0.1:5000/api/calendar/${userId}`);
      if (calRes.ok) setUpcomingAssignments(await calRes.json());

      // 2. Fetch Upcoming Tasks Data (Only MAIN tasks with calculated progress)
