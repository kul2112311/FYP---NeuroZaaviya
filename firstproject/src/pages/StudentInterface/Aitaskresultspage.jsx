...
export function AITaskResultsPage({ taskData, onBack, onEditPrompt }) {
  ...
  useEffect(() => {
      if (!user?.id) return;
      fetch(`${import.meta.env.VITE_API_URL}/api/calendar/${user.id}`)
        .then(res => res.json())
  ...
  const handleConfirmAndAdd = async () => {
    ...
    try {
      ...
      const payload = {
        studentId: user?.id, // Forced Ushna's ID directly to prevent context errors
        title: taskData?.aiResult?.parent_title || "AI Task Breakdown",
                  ...
                  {Array.from({ length: 14 }, (_, i) => 8 + i).map(hour => (
                    <div key={hour} className="flex gap-1">
                      ...
                      {weekDates.map((day, dayIdx) => {
                        ...
                        // Check if you just dragged a subtask here
                        const scheduledHere = Object.entries(scheduledSubtasks).find(
                          ([, s]) => s.date === dateKey && parseInt(s.time.split(":")[0]) === hour
                        );

                        // ✨ NEW: Check if the database already has something here!
                        const existingHere = existingEvents.find(e => {
                          if (!e.dueDate || !e.time) return false;
                          return e.dueDate === dateKey && parseInt(e.time.split(":")[0]) === hour;
                        });
                        ...
                        return (
                          <div key={dayIdx}
                            ...
                            style={{ 
                              ...
                              // Paint it purple if newly scheduled, or light gray if already booked
                              background: scheduledHere ? "#e8eaf6" : existingHere ? "#f5eef8" : "transparent" 
                            }}
                            onDragOver={handleDragOver}
                            onDrop={e => handleDrop(e, day, hour)}>
                            
                            {/* Render the NEW subtask you just dropped */}
                            {scheduledHere && (() => {
                              const [subtaskId, info] = scheduledHere;
                              return (
                                <div className="absolute inset-0 p-1 flex flex-col justify-between z-10" style={{ background: "#e8eaf6", borderRadius: "8px" }}>
                            ...
                            {/* ✨ NEW: Render the EXISTING blocked slot so you don't double book! */}
                            {existingHere && !scheduledHere && (
                              <div className="absolute inset-0 p-1 flex flex-col justify-center items-center opacity-60 cursor-not-allowed">
                                 <div className="text-[8px] font-bold uppercase tracking-wider mb-0.5" style={{ color: "#b39ddb" }}>Booked</div>
                                 <div className="text-[9px] font-medium truncate w-full text-center" style={{ color: "#5a4a61" }}>{existingHere.title}</div>
                              </div>
                            )}
                          </div>
                        );
