...
import { useUser } from '../../styles/SignInLandingPage/usercontext.jsx'; // Importing user context to get the logged-in user's ID

export function AITaskBreakdownScreen({ onBack, onAnalyze, savedTaskData,initialPrompt = "", initialTitle = "", initialFile = null}) {
  const [taskPrompt, setTaskPrompt] = useState(savedTaskData?.taskPrompt || initialPrompt);
  ...
  const { user } = useUser();
  const [rubricFile, setRubricFile] = useState(savedTaskData?.rubricFile || initialFile);
