...
export function FocusPeerManagement() {
  ...
  const commitApprove = async (application) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/requests/approve/${application.id}`, { method: 'POST' });
      if (response.ok) {
        setEmailModal(null);
  ...
  const commitReject = async (appId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/requests/reject/${appId}`, { method: 'POST' });
      if (response.ok) {
        setEmailModal(null);
        fetchData(); // Refresh the lists!
      }
